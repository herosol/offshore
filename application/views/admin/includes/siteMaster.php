...
<head>
    ...
    <link type="text/css" rel="stylesheet" href="<?=base_url()?>assets/css/font-icon.min.css?v-<?=$adminsite_setting->site_version?>">
...
<body class="page-body  skin-white">
    ...
    <script src="<?= base_url('adminassets/js/pick-a-color-1.2.3.min.js'); ?>"></script>
        ...
        <?php if (isset($enable_timepicker)): ?>
        ...
        <script type="text/javascript">
            ...
            jQuery(".repeater-add-btn").click(function(){
                var clonedRow = jQuery(this).parents('#repeater').find('.items:last-child').clone();
                clonedRow.find('input').val('').end();
                clonedRow.find('textarea').val('').end();
                clonedRow.find('.repeater-remove-btn').html('<button id="remove-btn" class="btn btn-danger" type="button">Remove</button>');
                clonedRow.find('img').attr('src',base_url+'assets/images/no_image.jpg');
                jQuery(this).closest('#repeater').before().append(clonedRow);
            });
            jQuery(document).on('click', '.repeater-remove-btn button', function () {
        ...
        <?php
            if($enable_vehcile_add_scripts==true){
        ?>
            ...
            <script src="<?=base_url()?>assets/js/jquery.min.js?v-<?=$site_settings->site_version?>"></script>
...
<script type="text/javascript" src="<?=base_url()?>assets/js/toastr.min.js?v-<?=$site_settings->site_version?>"></script>
            ...
            <script type="text/javascript" src="<?= base_url('assets/js/admin_custom.js?v-' . $site_settings->site_version) ?>"></script> 
            ...
            <script>
                ...
                var areas = Array('extras', 'guideline');
